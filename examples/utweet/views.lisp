...
(defview profile (username)
  (let* ((user (single '<user> :username username))
         ...
         (is-self (equal (get-userid req) username)))
    (render-template eco-template:profile (get-userid req) user
                     avatar-url user-tweets is-self)))

@route app "/followers/:username"
(defview user-followers (username)
  (let ((user (single '<user> :username username)))
    (render-template eco-template:user-list (get-userid req) "Followers"
                     user (follower-summary user))))

@route app "/following/:username"
(defview user-following (username)
  (let ((user (single '<user> :username username)))
    (render-template eco-template:user-list (get-userid req) "Following"
...
(defview sign-up ()
  (with-params req (name username email password password-repeat)
...
(defview sign-in ()
  (with-params req (username password)
...
(start app
    :middleware ((clack.middleware.static:<clack-middleware-static>  
                  :path "/static/"
